...
export async function getRecipeSubCategory(filteredValues, id, setIsLoading) {
  try {
    setIsLoading(true);
...
export async function addRecipeSubCategory(
  name,
  description,
  image,
  category_id
...
export async function eidtRecipeSubCategory(
  name,
  description,
  image,
  category_id,
...
export async function getRecipeSubCategoryById(id) {
  try {
    const res = await axios.get(`${domain}/api/v1/store/recipe_category/${id}`, {
      headers: {
        Authorization: `Bearer ${Token}`,
      },
    });
...
export async function getRecipeSubCategoryFilterById(
